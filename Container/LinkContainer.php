...
namespace Kaikmedia\EventsModule\Container;
...
class LinkContainer implements LinkContainerInterface
{
    ...
    private function getAdmin()
    {
        ...
        if ($this->accessManager->hasPermission(ACCESS_EDIT, false, ':manager:list')) {
            $links[] = [
                    'url' => $this->router->generate('kaikmediaeventsmodule_manager_list'),
                    'text' => $this->translator->__('List'),
                    'title' => $this->translator->__('Events manager list'),
                    'icon' => 'list'];
            }
            if ($this->accessManager->hasPermission(ACCESS_ADD, false, ':manager:modify')) {
                $links[] = [
                    'url' => $this->router->generate('kaikmediaeventsmodule_manager_modify'),
                    'text' => $this->translator->__('New'),
                    'title' => $this->translator->__('Add a new Event'),
                    'icon' => 'plus'];
            }
            if ($this->accessManager->hasPermission(ACCESS_ADMIN, false)) {
                $links[] = [
                    'url' => $this->router->generate('kaikmediaeventsmodule_admin_preferences'),
            ...
            if ($this->accessManager->hasPermission(ACCESS_ADMIN, false)) {
                $links[] = [
                    'url' => $this->router->generate('kaikmediaeventsmodule_import_index'),
