...
(function ($) {
    Kaikmedia.Events.import = (function () {
        // Init
        var data = {
            imported: 0,
            rejected: 0
        ...
        function tableCheck()
        {
            importAjax(Routing.generate('kaikmediaeventsmodule_import_status'), data).done(function (result) {
                data = result;
...
//            // call import
        ...
        function calcEvents() {
            if (data.total > 0) {
                data.eventSize = settings.limit;
                data.events = Math.ceil(data.total / data.eventSize);
        ...
        function itemsImport(data) {
            ...
            def.progress(function (data) {
                ...
                var percent = 100 * data.event / data.events;
                $("#import_progress").find('.progress-bar').css('width', percent + '%').attr('aria-valuenow', percent);
                $("#import_progress").find('.info').text('Importing ' + data.event + ' event from ' + data.events).css('color', '#000');
            });
            data.event = 0; // first event 0-49
            calcEvents(); // once again
            (function loop(data, def) {
                if (data.event < data.events || data.events === 0) {
                    importAjax(Routing.generate('kaikmediaeventsmodule_import_import'), data).done(function (data) {
                        data.event++;
                        def.notify(data);
                        loop(data, def);
                    });
